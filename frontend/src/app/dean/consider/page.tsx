...
export default function DeanApprovalPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const TARGET_STATUS_ID = 4; // 4 = รอคณบดีพิจารณา
  ...
  const handleApprove = async () => {
    ...
    if (result.isConfirmed) {
      const NEXT_STATUS_ID = 6; // 6 = รอคณบดีพิจารณา
  ...
  const handleConfirmReject = async () => {
    ...
    if (selectedId && selectedItem) {
      const displayName = getDisplayName(selectedItem);
      const REJECT_STATUS_ID = 7; // 7 = คณบดีตีกลับ
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-slate-500 mt-2 font-medium flex items-center gap-2">
                <Building2 className="w-4 h-4" /> สำหรับคณบดี
              </p>
            </div>
          </div>
