...
export interface Nomination {
  ...
  // ข้อมูลที่ได้จากการยิงดึง Detail มาประกอบ
          ...
          >
            ...
            <div className="grid grid-cols-7 gap-y-1.5 gap-x-1">
              {Array.from({ length: new Date(viewDate.getFullYear(), viewDate.getMonth(), 1).getDay() }).map((_, i) => (
                <div key={`blank-${i}`} className="w-full h-10"></div>
              ))}
...
// 2. Main Component
// ==========================================
export default function StudentDevelopmentCommitteeHistoryPage() { 
  ...
  const [sortConfig, setSortConfig] = useState<{ key: string | null, direction: 'asc' | 'desc' | null }>({ key: 'voted_at', direction: 'desc' });
  ...
  const getStatusBadge = (operation: string) => {
      ...
      } else if (op === "approve" || op === "approved") {
  ...
  const getDisplayName = (item: Nomination) => {
    if (!item.student_firstname && !item.student_lastname) return "รอข้อมูลจากระบบ";
  ...
  // ==========================================
  // 3. Data Fetching & Mapping Details
  // ==========================================
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // 1. ดึงข้อมูล Vote Logs จาก API (มีแค่ form_id, user_id, operation, voted_at)
        const response = await api.get(`/awards/my/vote-logs`, { params: { limit: 3000 } });
        const rawVoteLogs = response.data?.data || [];

        // 2. ใช้ Promise.all วนลูปไปยิง API ขอดึง Details ของแต่ละฟอร์ม
        const mappedDataPromises = rawVoteLogs.map(async (item: any) => {
            try {
                // ยิงไปดึงข้อมูลเต็มของฟอร์มนั้นๆ
                ...
                return {
                    ...item, // ข้อมูลจาก vote logs
                    
                    // นำข้อมูลที่ได้จาก Details มาประกอบ
                    student_firstname: detailData.student_firstname,
                    student_lastname: detailData.student_lastname,
                    student_number: detailData.student_number,
                    student_year: detailData.student_year,
                ...
                // ถ้าดึง detail ไม่ผ่าน ก็คืนค่าที่พอมีไปก่อน
                return {
                    ...item,
                    award_type_name: "-",
        ...
        // รอจนกว่าจะดึงข้อมูลครบทุกฟอร์ม
  ...
  const processedData = useMemo(() => {
    ...
    if (filterDate) {
      ...
      filtered = filtered.filter(item => {
          const itemTime = new Date(item.voted_at).getTime(); 
    ...
    if (sortConfig.key) {
      filtered.sort((a: any, b: any) => {
        ...
        } else if (sortConfig.key === 'voted_at') { 
          valA = new Date(a.voted_at).getTime();
          valB = new Date(b.voted_at).getTime();
        }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white shadow-sm rounded-3xl p-8 relative z-50">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-slate-500 mt-2 font-medium flex items-center gap-2">
                ...
                สำหรับคณะกรรมการพิจารณานิสิตดีเด่น
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-4 w-full">
            <div className="relative w-full flex-1">
              ...
              <input 
                type="text" placeholder="ค้นหาชื่อ, รหัสนิสิต หรือ รางวัล..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} 
            ...
            <CustomDatePicker label="วันที่พิจารณา (ถึงวันที่)" value={filterDate} onChange={setFilterDate} disabled={loading} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden relative z-10">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 text-slate-500 text-xs font-bold uppercase tracking-wider border-b border-slate-200">
                  ...
                  <th className="p-5 cursor-pointer hover:bg-slate-100 transition-colors text-center" onClick={() => handleSort('voted_at')}>
                     <div className="flex items-center justify-center gap-1">วันที่พิจารณา {sortConfig.key === 'voted_at' ? (sortConfig.direction === 'asc' ? <ArrowUp className="w-3 h-3 text-emerald-500"/> : <ArrowDown className="w-3 h-3 text-emerald-500"/>) : <ArrowUpDown className="w-3 h-3 text-slate-300"/>}</div>
                  </th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="p-16 text-center">
                      <div className="flex flex-col items-center justify-center text-slate-400">
                        <div className="bg-slate-50 p-4 rounded-full mb-3"><CheckCircle2 className="w-10 h-10 text-slate-300" /></div>
                  ...
                  currentItems.map((item, index) => (
                    <tr key={`${item.vote_log_id || index}-${index}`} className="transition-colors hover:bg-slate-50 group cursor-pointer animate-fade-in-up" style={{ opacity: 0, animationDelay: `${index * 50}ms`, animationFillMode: 'forwards' }}>
                      <td className="p-5">
                        <div className="flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full shrink-0 bg-slate-300 group-hover:bg-emerald-500 transition-colors"></div>
                           <div>
                             <p className={`text-sm font-bold ${item.student_firstname ? 'text-slate-800' : 'text-slate-400'}`}>
                                {getDisplayName(item)}
                             </p>
                             {item.student_year && <p className="text-xs text-slate-500">ปี {item.student_year}</p>}
                           </div>
                        </div>
                      </td>
          ...
          <div className="bg-slate-50 border-t border-slate-200 p-5 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="text-sm text-slate-500 font-medium">พบข้อมูลทั้งหมด {totalItems} รายการ</div>
          </div>
        </div>
