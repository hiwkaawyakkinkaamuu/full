...
type SearchAwardRequest struct {
	...
	SortBy      string `query:"sort_by"`      // name, studentNumber, awardType, date (default: date)
...
type SearchApprovalLogRequest struct {
	...
	SortBy      string `query:"sort_by"`     // name, studentNumber, academicYear, date (default: date)
	SortOrder   string `query:"sort_order"`  // asc, des/desc (default: des)
...
type CreateAwardTypeLogRequest struct {
	FormID       uint   `json:"form_id"`
...
type SearchAwardTypeLogRequest struct {
	Keyword   string `query:"keyword"`
	SortOrder string `query:"sort_order"`
	Arrange   string `query:"arrange"`
}

type SearchVoteLogRequest struct {
	Keyword string `query:"keyword"` // ชื่อ-นามสกุล หรือรหัสนิสิต
	Date    string `query:"date"`    // วันที่โหวต (format: YYYY-MM-DD)
	Page    int    `query:"page"`    // default: 1
}

type AwardTypeLogResponse struct {
	TypeLogID    uint      `json:"type_log_id"`
	FormID       uint      `json:"form_id"`
	UserID       uint      `json:"user_id"`
...
type AnnouncementAwardRequest struct {
	AcademicYear int    `query:"academic_year"`
	Semester     int    `query:"semester"`

	KeywordExtracurricular string `query:"keyword_extracurricular"`
	KeywordCreativity      string `query:"keyword_creativity"`
	KeywordBehavior        string `query:"keyword_behavior"`
	KeywordOther           string `query:"keyword_other"`

	PageExtracurricular int `query:"page_extracurricular"`
	PageCreativity      int `query:"page_creativity"`
	PageBehavior        int `query:"page_behavior"`
	PageOther           int `query:"page_other"`
}

type AnnouncementAwardItem struct {
	FormID           uint   `json:"form_id"`
	CampusID         int    `json:"campus_id"`
	AcademicYear     int    `json:"academic_year"`
	Semester         int    `json:"semester"`
	AwardType        string `json:"award_type"`
	AwardTypeGroup   string `json:"award_type_group"`
	FacultyID        int    `json:"faculty_id"`
...
type AwardTypeOption struct {
	Label   string `json:"label"`
	Value   string `json:"value"`
	IsOther bool   `json:"is_other"`
}

type AnnouncementAwardFilterMeta struct {
	AcademicYear     int               `json:"academic_year"`
	AcademicYears    []int             `json:"academic_year_options,omitempty"`
	Semester         int               `json:"semester"`
	Semesters        []int             `json:"semester_options,omitempty"`
	AwardType        string            `json:"award_type"`
	FacultyID        int               `json:"faculty_id,omitempty"`
	Keyword          string            `json:"keyword,omitempty"`
	AwardTypeOptions []AwardTypeOption `json:"award_type_options"`
}

type PaginatedAnnouncementAwardResponse struct {
	Data       []AnnouncementAwardItem     `json:"data"`
	Pagination PaginationMeta              `json:"pagination"`
	Filters    AnnouncementAwardFilterMeta `json:"filters"`
	Sections   []AnnouncementAwardSection  `json:"sections"`
}

type AnnouncementAwardSection struct {
	Key        string                  `json:"key"`
	Label      string                  `json:"label"`
	Keyword    string                  `json:"keyword,omitempty"`
	Data       []AnnouncementAwardItem `json:"data"`
