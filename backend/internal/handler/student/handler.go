...
func (h *StudentHandler) CreateStudent(c *fiber.Ctx) error {
	...
	// Set isfirstlogin = false ถ้าเป็นการสร้างโดย admin (ไม่ใช่ register) 1/3/2026 ถ้าพังก็ลบอันนี้
	if student.UserID != 0 {
		// update user field
		if student.User.UserID == 0 {
			// กรณี preload User ไม่มา ให้ดึงใหม่
			userRepo := h.service.(interface {
				GetUserRepo() interface {
					UpdateUserFields(ctx interface{}, userID uint, updates map[string]interface{}) (*models.User, error)
				}
			})
			if userRepo != nil {
				_, _ = userRepo.GetUserRepo().UpdateUserFields(c.Context(), student.UserID, map[string]interface{}{"is_first_login": false})
			}
		} else {
			// ถ้ามี preload User
			userRepo := h.service.(interface {
			...
			if userRepo != nil {
				_, _ = userRepo.GetUserRepo().UpdateUserFields(c.Context(), student.User.UserID, map[string]interface{}{"is_first_login": false})
			}
		}
	}
